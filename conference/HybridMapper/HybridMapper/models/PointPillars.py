...
def _PointPillars(model):
    ...
    def _pillar_encoder(in_channels, out_channels):
        id_ = model.reserveDataId(2)
        model.appendLayer(LayerConv([num_batches * num_pillars, in_channels, out_channels, num_points,
                                     1, 1, 1, 1, 1, 1],
                                    [-1, -1, id_, id_ + 1]))

    def _backbone(in_channels, out_channels_list: list, num_layers_list: list):
        id_ = model.reserveDataId(100)
    ...
    def _neck(in_channels_list: list, out_channels):
        id_ = model.reserveDataId(100)
        oh, ow = int(H / 2), int(W / 2)
        kh, kw = 1, 1
    ...
    def _head(in_channels, n_anchors, n_classes):
        id_ = model.reserveDataId(10)
        oh, ow = int(H / 2), int(W / 2)
    ...
    _pillar_encoder(channels, features)
    _backbone(features, [features, features * 2, features * 4], [3, 5, 5])
    _neck([features, features * 2, features * 4], features * 2)
    _head(features * 2 * 3, num_classes * 2, num_classes)
