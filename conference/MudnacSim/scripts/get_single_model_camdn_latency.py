...
FILENAME_RE = re.compile(
    r'^(?P<modelName>\w+)_(?P<system>\w+)_dram-(?P<channels>\d+)_noc-(?P<noc_flit_size>\d+)_(?P<acc_num>\d+)acc_(?P<spm_total_mb>\d+)KB_batch-(?P<totalBatch>\d+)\w+)$'
)
...
def collect_data(directory: Path):
    ...
    for p in directory.iterdir():
        ...
        fields = {k: fields[k] for k in ['modelName', 'channels', 'noc_flit_size', 'acc_num', 'spm_bank_size', 'totalBatch', 'latency']}
        data.append(fields)
...
def sort_data(data):
    """Sort data by modelName, channels, noc_flit_size, acc_num, spm_bank_size, totalBatch."""
...
def print_to_console(data):
    ...
    print('-' * 120)
    print(f"{'modelName':<10} {'channels':<8} {'noc_flit_size':<12} {'acc_num':<7} {'spm_bank_size':<12} {'totalBatch':<10} {'latency':<10}")
    print('-' * 120)
    for row in data:
        print(f"{row['modelName']:<10} {row['channels']:<8} {row['noc_flit_size']:<12} {row['acc_num']:<7} {row['spm_bank_size']:<12} {row['totalBatch']:<10} {row['latency']:<10}")
...
def write_to_csv(data, csv_path):
    ...
    fieldnames = ['modelName', 'channels', 'noc_flit_size', 'acc_num', 'spm_bank_size', 'totalBatch', 'latency']
...
def main():
    ...
    parser.add_argument('--dir', '-d', default='expr/output/expr_single_model/script_output/mudnacsim',
                        help='Directory to scan')
    parser.add_argument('--csv', '-c', default='single_model_latencies_camdn.csv',
