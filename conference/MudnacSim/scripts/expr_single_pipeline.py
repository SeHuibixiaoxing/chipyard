...
# spmBankList = [256,512,1024,2048] # KB
spmBankList = [256,512,1024,2048] # KB
...
method_list = ["ours2", "gemini2", "tangram2"]
# method_list = ["ours2"]
...
def log_timeout(args, timeout=TASK_TIMEOUT):
    """记录任务超时状态"""
    # 解包参数
    system, method, modelName, totalBatch, acc_num, spm_bank_size, mappingPath, pipeStrategy, pipeAccStrategy, noc, dramChannels, noc_flit_size, debug_flags = args
    
    # 创建参数摘要
    param_summary = (
        f"Model: {modelName}, Method: {method}, TotalBatch: {totalBatch}, Acc: {acc_num}, SPM Banks: {spm_bank_size}, "
...
def log_completion(args):
    ...
    # 创建参数摘要
    param_summary = (
        f"Model: {modelName}, Method: {method}, TotalBatch: {totalBatch}, Acc: {acc_num}, SPM Banks: {spm_bank_size}, Mapping: {mappingPath}, "
...
def run_command(cmd, timeout, stdout_path=None, stderr_path=None):
    ...
    try:
        ...
        return -1, '', str(e)
def run(system, method, modelName, totalBatch, acc_num, spm_bank_size, mappingPath, pipeSpmStrategy, pipeAccStrategy, noc, channels, noc_flit_size, debug_flags):
    # Build a concise output name
    outputName = f"{modelName}_{system}_dram-{channels}_noc-{noc}-{noc_flit_size}_{acc_num}acc_{spm_bank_size*acc_num}KB_{pipeSpmStrategy}_{pipeAccStrategy}_batch-{totalBatch}_method-{method}"
                                ...
                                for pipeStrategy in pipeSpmStrategyList:
                                    for pipeAcc in pipe_acc_strategy_list:
                                        for totalBatch in batch_size:
                                            mappingPath = f"models/pipeline/{modelName}/entire_model/{acc_num}_{spm_bank_size}_{totalBatch}_{dramBWPerChannel}_{noc_flit_size}_{method}.yaml"
                                            with open(mappingPath, 'r', encoding='utf-8') as file:
                                                mapping = yaml.safe_load(file) 
                                                if int(mapping['cost']) > 1e13:
                                                    print(f"skip mapping: {mappingPath}, cost: {mapping['cost']}")
                                                    continue
                                            argList.append((system, method, modelName, totalBatch, acc_num, spm_bank_size, mappingPath, pipeStrategy, pipeAcc, noc, dram_channel, noc_flit_size, debug_flags))
