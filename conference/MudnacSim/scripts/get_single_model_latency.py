...
FILENAME_RE = re.compile(
    r'^(?P<modelName>\w+)_(?P<system>\w+)_dram-(?P<channels>\d+)_noc-(?P<noc>\w+)-(?P<noc_flit_size>\d+)_(?P<acc_num>\d+)acc_(?P<spm_total_mb>\d+)KB_(?P<pipeSpmStrategy>\w+)_(?P<pipeAccStrategy>\w+)_batch-(?P<totalBatch>\d+)_method-(?P<method>\w+)$'
)

LATENCY_RE = re.compile(r'Total Latency:\s*(\d+)', re.IGNORECASE)

# Fields used for CSV and generating combinations
...
def expand_combinations(data):
    """Ensure all combinations of non-latency fields exist in data.

    For any missing combination, append a row with `latency` set to empty string.
    """
    if not data:
        return data

    # Fields to consider for combinations (exclude 'latency')
    keys = [f for f in FIELDNAMES if f != 'latency']
    ...
    # Build existing key set for fast lookup (stringified)
    existing = set()
    for row in data:
        existing.add(tuple(str(row[k]) for k in keys))

    # Generate Cartesian product of all unique values
    pools = [sorted(uniques[k], key=str) for k in keys]
    for combo in itertools.product(*pools):
        key = tuple(str(v) for v in combo)
        if key in existing:
            continue
        # Create new row preserving original value types where possible
        new_row = {k: combo[i] for i, k in enumerate(keys)}
        new_row['latency'] = ''
        data.append(new_row)
        existing.add(key)
...
def main():
    ...
    parser.add_argument('--dir', '-d', default='expr/output/expr_single_pipeline/script_output/mudnacsim',
                        help='Directory to scan')
    parser.add_argument('--csv', '-c', default='single_model_latencies.csv',
                        help='Output CSV file path')
    args = parser.parse_args()

    directory = Path(args.dir)
    data = collect_data(directory)
    # Ensure all combinations exist (missing entries get empty latency)
    data = expand_combinations(data)
