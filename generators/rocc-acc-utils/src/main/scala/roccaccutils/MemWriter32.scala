...
class MemWriter32(val cmd_que_depth: Int = 4, val write_cmp_flag:Boolean = true, val logger: Logger = DefaultLogger)(implicit p: Parameters, val hp: L2MemHelperParams)
  ...
  private def largestPowerOfTwoLE(value: UInt): UInt = {
    val result = Wire(UInt((BUS_SZ_BYTES_LG2UP + 1).W))
    result := 0.U
    for (shift <- BUS_SZ_BYTES_LG2UP to 0 by -1) {
      when(value(shift)) {
        result := (1 << shift).U
      }
    }
  ...
  private def log2OfPowerOfTwoOrZero(value: UInt): UInt = {
    val result = Wire(UInt(BUS_SZ_BYTES_LG2UP.W))
    result := 0.U
    for (shift <- 0 to BUS_SZ_BYTES_LG2UP) {
      when(value === (1 << shift).U) {
        result := shift.U
      }
    }
    result
  ...
  val throttle_end_uncapped = Mux(buf_lens_Q.io.deq.valid,
    buf_lens_Q.io.deq.bits - backend_bytes_written,
    BUS_SZ_BYTES.U)

  val throttle_end = Mux(throttle_end_uncapped >= BUS_SZ_BYTES.U, BUS_SZ_BYTES.U, throttle_end_uncapped)

  val throttle_end_writeable = largestPowerOfTwoLE(throttle_end)
  val throttle_end_writeable_log2 = log2OfPowerOfTwoOrZero(throttle_end_writeable)

  val ptr_align_max_bytes_writeable = Wire(UInt((BUS_SZ_BYTES_LG2UP + 1).W))
  ptr_align_max_bytes_writeable := BUS_SZ_BYTES.U
  for (shift <- (BUS_SZ_BYTES_LG2UP - 1) to 0 by -1) {
    when(backend_next_write_addr(shift)) {
      ptr_align_max_bytes_writeable := (1 << shift).U
    }
  }

  val ptr_align_max_bytes_writeable_log2 = log2OfPowerOfTwoOrZero(ptr_align_max_bytes_writeable)

  val count_valids_largest_aligned = largestPowerOfTwoLE(count_valids)
  val count_valids_largest_aligned_log2 = log2OfPowerOfTwoOrZero(count_valids_largest_aligned)
