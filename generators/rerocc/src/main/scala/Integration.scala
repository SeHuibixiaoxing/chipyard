...
trait CanHaveReRoCCTiles { this: BaseSubsystem with InstantiatesHierarchicalElements with constellation.soc.CanHaveGlobalNoC =>
  ...
  val reRoCCManagers = p(ReRoCCTileKey).zipWithIndex.map { case (g,i) =>
    ...
    sbus.coupleTo(s"sport_named_rerocc_$i") {
      (rerocc_tile.stlNode :*= TLBuffer() :*= TLWidthWidget(sbus.beatBytes) :*= TLBuffer() :*= _)
    }
    if (g.connectSbusSlaveToStl) {
      sbus.coupleTo(s"sport_named_rerocc_sbus_$i") {
        rerocc_tile.rocc.sbusSlaveTLNode :*= TLWidthWidget(sbus.beatBytes) :*= _
      }
    }
