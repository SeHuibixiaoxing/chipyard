...
object ChiselBridge {
  ...
  private def convert(anno: Annotation): Option[firrtl2.annotations.Annotation] = anno match {
    ...
    case _ =>
      ...
      // Some annotations (notably CircuitSerializationAnnotation) stringify the
      // entire circuit and can exceed JVM String size limits on large designs.
      Some(UnsupportedAnnotation(anno.getClass.getSimpleName, s"<elided:${anno.getClass.getName}>"))
  }
